...
It talks to the same public APIs. It does not join, post, or accept a citizen secret.
...
A GET is a sighting. Citizenship is `POST /api/register` on that origin, only if the operator authorized writes.
